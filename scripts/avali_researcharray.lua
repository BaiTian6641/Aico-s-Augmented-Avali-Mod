-- Avali Research Processor Array (compact)
-- Advanced FU-style examiner: insert ANY item, computing units boost speed & rewards
...
-- Full tag list + generic fallback
local tagList = {
  ...
  generic          = { range=100, currencies={ bonusResearch=4 } }
}

local COMPUTE_UNIT_EFF  = 0.6
local COMBIN_UNIT_EFF   = 1.8
local COMPUTE_MULT_PER  = 0.3
...
function init()
  ...
  -- Slot layout (6 slots): itemGrid shows input+compute, itemGrid2 shows outputs
  -- [0:Input] [1:Comp1] [2:Comp2]  |  [3:Research] [4:Essence] [5:Output]
  self.inputSlot    = 0
  self.computeMin   = 1
  self.computeMax   = 2
  self.researchSlot = 3
...
function countOccupiedSlots()
  local used = 0
  local sc = config.getParameter("slotCount", 6)
  for i = 0, sc - 1 do
    ...
    if storage.currentItem then
      if not storage.finishedProcessing then
        ...
        if storage.futureItem then
          ...
          spawnBonus("essence", storage.bonusEssence, self.essenceSlot)
