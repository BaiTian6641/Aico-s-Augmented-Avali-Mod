...
-- FU-style examiner: insert tagged items to research, computing units boost speed & rewards
...
-- Tag-based reward definitions (subset for tier-2 processor array)
-- FORMAT: tag = { range=randMax, currencies={ bonusName=value, ... }, overrideCategory? }
local tagList = {
...
local COMPUTE_MULT_PER  = 0.25  -- Extra multiplier per tick per unit of compute efficiency
...
function init()
  ...
  self.computeMax  = 4   -- last computing unit slot (4 slots: 1-4)
  self.researchSlot= 5   -- research currency output
  self.essenceSlot = 6   -- essence currency output
  self.protheonSlot= 7   -- protheon currency output
  self.geneSlot    = 8   -- genetic material output
  self.outputSlot  = 9   -- examined item output
  ...
  if playerUsing or selfWorking then
    if storage.currentItem then
      if not storage.finishedProcessing then
        -- Efficiency gets computing unit boost
        local effectiveEff
        if playerUsing then
        ...
        -- Multiplier: base 1 per tick + bonus from computing units
